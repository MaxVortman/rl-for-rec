...
class TransformerModel(nn.Module):
    ...
    def __init__(self, ntoken: int, d_model: int, nhead: int, d_hid: int,
                 nlayers: int, dropout: float = 0.5, padding_idx:int = 0):
        super().__init__()
        self.model_type = 'Transformer'
        self.d_model = d_model

        self.embedding = nn.Embedding(ntoken + 1, d_model, padding_idx=padding_idx) # + padding_idx
    ...
    def forward(self, src: Tensor, src_mask: Tensor, src_key_padding_mask: Tensor) -> Tensor:
        ...
        x = self.transformer_encoder(x, mask=src_mask, src_key_padding_mask=src_key_padding_mask) # (sequence length, batch_size, dim_model)
